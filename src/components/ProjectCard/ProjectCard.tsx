...
export const ProjectCard = ({
  ...
  const cardContent = (
    <>
      <Image
        ...
        width={300}
        height={200}
        className={`rounded-xs border-2 border-transparent duration-200 ${
          !isInternal ? "group-hover:border-slate-400" : ""
        }`}
      />
      <div className="flex flex-col gap-4 h-full justify-center">
        <h2 className={`text-lg font-bold flex gap-2 items-center ${
          !isInternal ? "group-hover:text-teal-600" : "text-slate-300"
        }`}>
          {title}
          {!isInternal && (
  ...
  if (isInternal || !href) {
    return (
      <div className={`flex items-center md:flex-row flex-col border-2 border-slate-400 rounded-xs border-opacity-20 
          gap-8 p-4 ${isInternal ? "opacity-80" : ""}`}>
        {cardContent}
      </div>
    );
  }
